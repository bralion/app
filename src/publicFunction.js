		...
		Vue.prototype.request = function ({
			...
			if(type){
				switch (type) {
					// 删除请求
					case 'delete':
						response=axios.delete(baseUrl + url, formData
							? formData
							: qs.stringify(data), headers)
						break
					// 修改请求
					case 'put':
						response=axios.put(baseUrl + url, formData
							? formData
							: qs.stringify(data), headers)
						break
					// 新增请求
					case 'put':
						response=axios.post(baseUrl + url, formData
							? formData
							: qs.stringify(data), headers)
						break
					// 查询请求
					case 'get':
						response=axios.get(baseUrl + url, formData
							? formData
							: qs.stringify(data), headers)
						break
				}
			...
			response.then(response => {//状态码200-300
